# STEG 2: Databasschema (Supabase/PostgreSQL)

## Instruktioner till Claude Code

Skapa SQL-migreringsfiler i `supabase/migrations/`. Kör dem i ordning via Supabase Dashboard SQL Editor eller CLI.

---

## 2.1 Huvudschema (`001_initial_schema.sql`)

Skapa filen `supabase/migrations/001_initial_schema.sql`:
...
## 2.2 Row Level Security (`002_rls_policies.sql`)

Skapa filen `supabase/migrations/002_rls_policies.sql`:
...
-- CV bucket policies: only recruiter who uploaded + company that owns the job can access
...
After running the migrations, generate types:

```bash
npx supabase gen types typescript --project-id YOUR_PROJECT_ID > src/types/database.ts
```

Alternativt, skapa typer manuellt i `src/types/database.ts` baserat på schemat ovan.

---

## Verifiering

Kolla i Supabase Dashboard:
- [ ] Alla tabeller skapade under Table Editor
- [ ] RLS aktiverat på alla tabeller (lås-ikon)
- [ ] Storage buckets: `cvs`, `avatars`, `logos`
- [ ] Trigger `on_auth_user_created` finns under Database → Triggers

**Gå vidare till:** [03-AUTH-SYSTEM.md](./03-AUTH-SYSTEM.md)
