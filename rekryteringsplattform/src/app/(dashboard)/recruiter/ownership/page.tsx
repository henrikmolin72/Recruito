...
import { getRecruiterOwnership } from "@/lib/ownership/actions";
import {
  Card,
  CardContent,
  CardHeader,
...
export default async function RecruiterOwnershipPage() {
  ...
  const ownership = await getRecruiterOwnership(recruiter.id);
  ...
  const expiredCount = ownership.filter(
    (o) => o.status === "expired" || getDaysRemaining(o.ownership_end) <= 0
  ).length;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Oversikt av dina kandidatskydd hos foretag
        </p>
      </div>
      ...
      <Card className="border-brand-200 bg-brand-50">
        <CardHeader className="pb-2">
          <CardTitle className="flex items-center gap-2 text-base">
            ...
            12 manaders kandidatskydd
          </CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground">
            Nar du presenterar en kandidat for ett foretag far du automatiskt 12
            manaders kandidatskydd. Om foretaget anstaller kandidaten inom denna
            period -- aven for en annan tjanst -- har du ratt till ditt arvode.
          </p>
        </CardContent>
      </Card>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-3xl font-bold text-gray-500">{expiredCount}</p>
          </CardContent>
        </Card>
      </div>
      ...
      {ownership.length > 0 ? (
        ...
        <EmptyState
          ...
          title="Inga kandidatskydd annu"
          description="Nar du presenterar kandidater for foretag skapas kandidatskydd automatiskt."
        />
      )}
    </div>
  );
