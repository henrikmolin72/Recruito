 ...
 *   500 — misconfiguration (missing RESEND_WEBHOOK_SECRET / RESEND_API_KEY), or
 *         a transient DB failure while suppressing (Svix retries for ~24h, so
 *         the suppression isn't lost)
...
export async function POST(request: NextRequest) {
  ...
  // The Resend constructor throws on a missing API key. Checked up front so that
  // throw can't land in the verification catch below and masquerade as a 400
  // "Invalid signature" — a 400 stops Svix retrying and would lose the event.
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) {
    console.error("RESEND_API_KEY not configured; rejecting webhook.");
    return NextResponse.json({ error: "Not configured" }, { status: 500 });
  }
