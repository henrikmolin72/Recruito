...
describe("POST /api/webhooks/resend", () => {
  ...
  // The real Resend constructor throws on a missing API key. That throw used to
  // land in the verification catch and surface as 400 "Invalid signature", which
  // tells Svix not to retry — silently discarding the bounce. A missing key is
  // misconfiguration, so it must be 500 (retryable) and never reach verify().
